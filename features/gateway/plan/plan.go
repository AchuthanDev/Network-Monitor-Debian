...
const NftablesTableName = "network_monitor_gateway"
...
func BuildDryRun(cfg gatewayconfig.Config) Plan {
	...
	result.WouldChange = append(result.WouldChange,
		...
		"create isolated gateway nftables table inet "+NftablesTableName,
	...
	if cfg.Gateway.DHCP.Enabled {
		result.WouldChange = append(result.WouldChange, "write dnsmasq DHCP-only configuration for monitored LAN only; do not bind DNS port 53")
	}
...
func RenderNftablesWithOptions(cfg gatewayconfig.Config, opts NftablesOptions) string {
	...
	for _, client := range opts.ClientCounters {
		...
		clientCounters.WriteString(fmt.Sprintf("  counter nm_gateway_%s_internet_download {}\n", name))
		clientCounters.WriteString(fmt.Sprintf("  counter nm_gateway_%s_internet_upload {}\n", name))
	}
	...
	for _, client := range opts.ClientCounters {
		...
		name := sanitizeCounterName(client.Name)
		clientRules.WriteString(fmt.Sprintf("    iifname \"%s\" oifname \"%s\" ip saddr %s ip daddr != @monitored_lan4 counter name nm_gateway_%s_internet_upload\n", cfg.Gateway.LANInterface, cfg.Gateway.WANInterface, client.IPv4, name))
		clientRules.WriteString(fmt.Sprintf("    iifname \"%s\" oifname \"%s\" ip daddr %s ip saddr != @monitored_lan4 counter name nm_gateway_%s_internet_download\n", cfg.Gateway.WANInterface, cfg.Gateway.LANInterface, client.IPv4, name))
	}
  ...
  counter nm_gateway_client_internet_download {}
  counter nm_gateway_client_internet_upload {}
  counter nm_gateway_client_lan_download {}
  counter nm_gateway_client_lan_upload {}
%s

  chain nm_gateway_prenat_account {
    type filter hook forward priority -150; policy accept;
%s    meta nfproto ipv4 iifname "%s" oifname "%s" ip saddr @monitored_lan4 ip daddr != @monitored_lan4 counter name nm_gateway_client_internet_upload
    meta nfproto ipv4 iifname "%s" oifname "%s" ip daddr @monitored_lan4 ip saddr != @monitored_lan4 counter name nm_gateway_client_internet_download
    meta nfproto ipv4 iifname "%s" ip saddr @monitored_lan4 ip daddr @monitored_lan4 counter name nm_gateway_client_lan_upload
    meta nfproto ipv4 oifname "%s" ip daddr @monitored_lan4 ip saddr @monitored_lan4 counter name nm_gateway_client_lan_download
  }
